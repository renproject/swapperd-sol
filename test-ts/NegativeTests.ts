...
import { randomID, secondsFromNow, Ox0, ETH, NULL } from "./helper/testUtils";
...
const testContract = (testCase: TestCase) => {
    ...
    contract(testCase.name, (accounts: string[]) => {
        ...
        it("order status checks in redeemToSpender", async () => {
            const swapID = randomID(), secret = randomID();
            const secretLock = `0x${SHA256(HEX.parse(secret.slice(2))).toString()}`;
            ...
            // Can only redeem for OPEN swaps and with valid key
            await swapperd.redeemToSpender(swapID, secretLock, { from: alice })
                .should.be.rejectedWith(null, /((revert)|(invalid secret))\.?$/);

            await swapperd.redeemToSpender(swapID, secret, { from: alice });
            await swapperd.redeemToSpender(swapID, secret, { from: alice })
                .should.be.rejectedWith(null, /((revert)|(swap not open))\.?$/);
            await swapperd.redeemToSpender(swapID, secret, { from: bob })
                .should.be.rejectedWith(null, /((revert)|(swap not open))\.?$/);
        });
        ...
        it("can't use 0x0 as spender", skip(["BaseSwap"], async () => {
            const swapID = randomID(), secret = randomID();
            const secretLock = `0x${SHA256(HEX.parse(secret.slice(2))).toString()}`;
            const value = 100000;
            await token.approve(swapperd.address, value, { from: alice });
            await swapperd.initiate(swapID, NULL, secretLock, await secondsFromNow(2), value, { from: alice, value: token === ETH ? value : 0 })
                .should.be.rejectedWith(null, /((revert)|(spender must not be zero))\.?$/);
            await swapperd.initiateWithFees(swapID, NULL, broker, 0, secretLock, await secondsFromNow(2), value, { from: alice, value: token === ETH ? value : 0 })
                .should.be.rejectedWith(null, /((revert)|(spender must not be zero))\.?$/);
        }));
    });
};
